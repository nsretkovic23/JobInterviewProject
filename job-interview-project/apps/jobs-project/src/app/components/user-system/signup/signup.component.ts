...
export class SignupComponent implements OnInit {
  ...
  constructor(private _ngZone: NgZone, private userFormBuilder:FormBuilder, private companyFormBuilder:FormBuilder) { }
  ...
  userSignupForm : FormGroup = this.userFormBuilder.group({
    ...
    userPicture : new FormControl('https://icon-library.com/images/account-icon-png/account-icon-png-2.jpg'),
    userDescription : new FormControl('',[Validators.required])
  })
  
  companySignupForm : FormGroup = this.companyFormBuilder.group({
    companyFullName : new FormControl('', [Validators.required, Validators.minLength(3)]),
    companyEmail : new FormControl('', [Validators.required, Validators.email]),
    companyPassword : new FormControl('', [Validators.required, Validators.minLength(6)]),
    // TODO: Change this so user can import his own picture instead of placing the link
    companyPicture : new FormControl('https://img.favpng.com/4/24/8/computer-icons-building-planacy-ab-architectural-engineering-biurowiec-png-favpng-4tXAHFuj3fEV7na0R5XfKBBz3.jpg'),
  ...
  onCompanySignUp(){
    ...
    {
      console.log("Invalid company inputs");
      return;
    }
  ...
  triggerResize() {
    ...
    this._ngZone.onStable.pipe(take(1))
        .subscribe(() => {
          if(this.autosizeUserDesc)
          this.autosizeUserDesc.resizeToFitContent(true)});
  }
